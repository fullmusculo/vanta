...
For direct structured tool calls from ChatGPT Work, run the local MCP bridge (`npm run editor:mcp`) and follow [the connection and security guide](docs/chatgpt-work-mcp.md). The bridge reuses the same project and revision checks as the editor; it does not require a ChatGPT password in Vanta.
