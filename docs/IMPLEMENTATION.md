...
Working local technical prototype on `feature/agentic-video-editor-mvp`, forked from `itsjwill/vanta` at `350b053ee18fb856a87516cfbfd1ad2295c26a73`. Real raw footage has now passed ingestion, perception and a 12-second portrait render. The full AI MVP acceptance gate is NOT complete: no API credentials are configured. Live ASR/Director calls and subjective editorial quality remain unverified. This is not a deployed SaaS.
...
- `POST /api/projects/:id/decisions`: accepts an interactive ChatGPT proposal with the current revision and editorial instruction, applies only allowlisted operations to the existing plan, and records the before/after and decision in history. The editor can export non-secret context and import the decision JSON. This workflow uses the human's ChatGPT session; it does not programmatically invoke a subscription from the worker.
...
`overlays[]`: title, lower-third, stat, image, broll or motion; output range, scale, position, existing source reference when needed. The `motion` type validates its preset (`kinetic-title`, `callout`, `stat-counter`), entrance/exit frames and optional numeric value. The same Remotion component drives preview and MP4, and the timeline inspector can change its text, position, duration and animation with undo/redo.
...
`profile`: configurable editorial preferences, font enum, colors, safe area, caption word count, transition preference, silence threshold, B-roll policy and optional logo asset. The FullMúsculo profile uses the verified `#8AE0FF` Cielo accent from the supplied brand book; `#040D20` and `#015AFF` are also specified there. The same book names Mona Sans Narrow as primary and Mona Sans Wide as secondary. Actual font files are not packaged, and motion rhythm/logo usage still await visual reference; `brandVerified` remains false. Existing plans snapshot their profile, so new defaults do not silently rewrite them.
...
- Codex CLI v0.156.1 reported `Logged in using ChatGPT` in the development environment. This grants Codex local-work access, not an OpenAI Platform API key for the app's speech/Responses calls. OpenAI documents separate API billing and recommends API-key authentication for programmatic Codex CLI workflows; the worker must not reuse the cached Codex login as an API credential. ChatGPT can still supply structured decisions interactively through the editor's import path.
...
- ChatGPT interactive-decision integration passed through API → revisioned project → Remotion render: a malformed object was rejected without mutation; a valid `set-clip` proposal changed an existing clip; stale revisions were rejected. The same route applied a modest visual-only punch-in to the uploaded phone footage, retaining the full 129-second source. This is an interactive chat-directed edit, not live unattended inference or transcription.
...
### ChatGPT Work MCP and real-video recovery — 2026-09-23

- Origin: `https://github.com/fullmusculo/vanta.git`; upstream (fetch only): `https://github.com/itsjwill/vanta.git`; branch: `feature/agentic-video-editor-mvp`; upstream baseline: `350b053ee18fb856a87516cfbfd1ad2295c26a73`.
- Added `server/mcp.ts` with seven local MCP tools for project context, sampled visual frames, validated decisions, timed transcript import, and asynchronous rendering/status. Reused the existing SQLite store, `EditPlan` validation, `applyDecision`, events and worker; no model-generated code or URLs execute. Added checkpoint importer and MCP smoke/CLI helpers. See `docs/chatgpt-work-mcp.md`.
- On the actual 129-second source, a ChatGPT-directed MCP decision added a short title and a four-frame fade at an existing silence cut. A second MCP decision moved the title away from the subject's face after visual review. Six silence cuts and 16 kept clips remain; project revision 9 retains both decisions and an editable plan.
- `npm run typecheck` passed; `npm test` passed 16/16. A real MCP client read the project and six images and rejected a stale revision plus an operation outside the allowlist. The resulting 126.314-second portrait MP4 was decoded end-to-end and sampled visually.
- Remotion render in this restricted runtime crashed near frame 3368 after earlier network-interface restrictions were worked around locally. The new export was recovered with FFmpeg from the previously verified Remotion render of the same silence-cut plan, applying the title and black fade. Project render metadata identifies this as `ffmpeg-postprocess`; the failed Remotion job remains recorded as failed. This recovery is specific to this media and does not establish a reliable general FFmpeg backend.
- ChatGPT Work plugin attachment still requires a reachable authenticated HTTPS endpoint or OpenAI Secure MCP Tunnel, which needs a Platform runtime key and outbound access to `api.openai.com`. This environment provides neither. The current ChatGPT session did exercise the local bridge through an MCP client, but the plugin is not installed in ChatGPT Work. No real ASR transcript, captions, music or newly generated images were produced; editorial speech analysis and the full MVP acceptance gate remain open.

### Motion graphics, optional local ASR and segmented Remotion render — 2026-09-23

- `src/agentic/MotionGraphic.tsx` adds three bounded, deterministic animated presets shared by Player and export. Their typed parameters are part of EditPlan and reject invalid combinations. The editor's graphics track selects them for editing in the inspector, with existing undo/redo and revisioned save. The Director can insert valid motion overlays through its `set-overlay` operation.
- The user supplied a FullMúsculo brand book. Its specified accent `#8AE0FF` is the new-project profile default, while the profile stays explicitly unverified until font assets, logo placement and visual pacing are approved. Existing project profiles remain unchanged. The render discussed here still has its earlier neutral profile.
- `server/whisper-cpp.ts` adds an optional local timed-word adapter. No executable, weights or live ASR success are claimed. `docs/ASR.md` covers configuration and the fallback import path. OpenAI Platform speech remains an independent option; the ChatGPT subscription login does not authorize API requests.
- `server/render.ts` divides long exports into 450-frame Remotion segments rendered with fresh browser processes and concatenates them with FFmpeg, resetting video timestamps to exact 30-fps frame positions. A two-segment 600-frame test produced 600 video frames and an exact 20.000-second video stream. This mitigates the Chromium failure previously seen on long footage; the full real-video verification is recorded separately once it completes.
- Typecheck, 19/19 contract/parser tests, editor build and `git diff --check` pass. The local motion preview for the real footage was rendered and visually inspected at its entrance and steady state. No licensed music, generated explanatory image, actual transcript, or speech-grounded captions have been supplied, so the full editorial acceptance gate remains open.
- The same real project was rendered fully through the worker after the preview. Its draft MP4 contains exactly **3,788 video frames**, 30 fps, **126.266667 seconds** at 540 × 960, plus 48 kHz stereo AAC at 126.272 seconds. FFmpeg decoded the entire video without error, and frames at 64 and 124 seconds were inspected. This establishes full-length Remotion export with motion graphics on this input, not final 1080 × 1920 quality. The job completed, project revision 11 includes its render record, and both the MP4 and editable project snapshot were saved as private deliverables. The original has approximately 129 seconds; six detected silence gaps account for the shorter cut. The output still lacks verified captions, music and explanatory imagery.
- Interactive browser verification could not run: the available cloud browser blocked localhost, while the local `agent-browser` CLI is absent. The API starts on loopback and the production UI bundles, but inspector interaction remains unverified in a browser.
